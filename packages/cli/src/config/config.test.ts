...
import { ShellTool, EditTool, WriteFileTool } from '@tiecode/tie-cli-core';
import { loadCliConfig, parseArguments, type CliArgs } from './config.js';
import type { Settings } from './settings.js';
import type { Extension } from './extension.js';
import * as ServerConfig from '@tiecode/tie-cli-core';
...
vi.mock('@tiecode/tie-cli-core', async () => {
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
    ...
    const extensions: Extension[] = [
      {
        ...
        contextFiles: ['/path/to/ext1/TIE.md'],
      },
      {
        path: '/path/to/ext2',
        config: {
  ...
  it.skip('should correctly use mocked homedir for global path', async () => {
    const MOCK_GEMINI_DIR_LOCAL = path.join('/mock/home/user', '.tie');
    const MOCK_GLOBAL_PATH_LOCAL = path.join(MOCK_GEMINI_DIR_LOCAL, 'TIE.md');
    mockFs({
