...
If you're looking for an issue to work on, check out our list of issues that are labeled ["help wanted"](https://github.com/qiaoliang/tie-cli/issues?q=is%3Aissue+state%3Aopen+label%3A%22help+wanted%22).
...
```bash
git clone https://github.com/qiaoliang/tie-cli.git # Or your fork's URL
cd gemini-cli
```
...
- Consult [GEMINI.md](https://github.com/qiaoliang/tie-cli/blob/main/GEMINI.md) (typically found in the project root) for specific instructions related to AI-assisted development, including conventions for React, comments, and Git usage.
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect gemini-cli due to automatic exclusion. Use `.tie/.env` files for gemini-cli specific debug settings.
...
Container-based sandboxing mounts the project directory (and system temp directory) with read-write access and is started/stopped/removed automatically as you start/stop Gemini CLI. Files created within the sandbox should be automatically mapped to your user/group on host machine. You can easily specify additional mounts, ports, or environment variables by setting `SANDBOX_{MOUNTS,PORTS,ENV}` as needed. You can also fully customize the sandbox for your projects by creating the files `.tie/sandbox.Dockerfile` and/or `.tie/sandbox.bashrc` under your project settings directory (`.tie`) and running `gemini` with `BUILD_SANDBOX=1` to trigger building of your custom sandbox.
